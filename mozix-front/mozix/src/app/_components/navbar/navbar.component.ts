...
import { SearchService } from '../../_services/search.service'; // Importáld a keresési szolgáltatást
...
export class NavbarComponent implements OnInit {
    ...
    searchQuery: string = ''; // Hozzáadva: keresési kifejezés tárolására

    constructor(private router: Router, private loginService: LoginService, private searchService: SearchService) { } // Injectáld a keresési szolgáltatást
        ...
        this.loginService.login(this.loginCredentials).subscribe({
            next: (response: LoginResponse) => {
                if (response.status === 'success' && response.statusCode === 200 && response.jwt) {
                    ...
                    localStorage.setItem('username', this.loginCredentials.username);
    ...
    // Új metódus a keresés indításához
    startSearch(): void {
        if (this.searchQuery.trim()) {
            this.router.navigate(['/search-results'], { queryParams: { q: this.searchQuery } });
            this.searchQuery = ''; // Töröljük a keresési kifejezést a sávból
        }
    }
}