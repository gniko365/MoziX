...
-- Procedures
--
DROP PROCEDURE IF EXISTS `actorsRoleWithImage`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `actorsRoleWithImage` ()   SELECT name, role, actor_image
FROM movie_actors
WHERE movie_id = 101$$

DROP PROCEDURE IF EXISTS `AllRoleOfActors`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AllRoleOfActors` ()   SELECT movie_id, role
FROM movie_actors
WHERE name = 'Sinkovits Imre'$$

DROP PROCEDURE IF EXISTS `Average_ratingsToAMovie`$$
...
DROP PROCEDURE IF EXISTS `BestRatedMovies`$$
...
DROP PROCEDURE IF EXISTS `BirthDateInOrder`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `BirthDateInOrder` ()   SELECT name, birth_date, role
FROM movie_actors
ORDER BY birth_date ASC$$

DROP PROCEDURE IF EXISTS `everyActorInAMovie`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `everyActorInAMovie` ()   SELECT movie_id, COUNT(*) AS actor_count
FROM movie_actors
GROUP BY movie_id
ORDER BY actor_count DESC$$

DROP PROCEDURE IF EXISTS `EveryMovieByADirector`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `EveryMovieByADirector` ()   SELECT 
    movie_id, 
    movie_name, 
    release_year
FROM movies
WHERE director_id = (SELECT director_id FROM directors WHERE name = 'Radványi Géza')$$

DROP PROCEDURE IF EXISTS `filmActorsBirthDateInOrder`$$
...
DROP PROCEDURE IF EXISTS `filmDetails`$$
...
DROP PROCEDURE IF EXISTS `MoviesByGenre`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `MoviesByGenre` ()   SELECT 
    movies.movie_id, 
    movies.movie_name, 
    movies.release_year
FROM movies
INNER JOIN genres ON movies.genre_id = genres.genre_id
WHERE genres.name = 'Vígjáték'$$

DROP PROCEDURE IF EXISTS `roles`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `roles` ()   SELECT name, role
FROM movie_actors
WHERE movie_id = 101$$
...
INSERT INTO `movies` (`movie_id`, `release_year`, `description`, `director_id`, `genre_id`, `movie_name`) VALUES
(101, 1948, 'Egy háború utáni történet gyermekekről, akik a túlélésért küzdenek.', 1, 1, 'Valahol Európában'),
(102, 1981, 'Egy színész, aki kompromisszumokat köt a karrierjéért.', 2, 1, 'Mephisto'),
(103, 1969, 'Egy férfi szürreális tapasztalatai a kommunista rezsim alatt.', 3, 2, 'A tanú'),
(104, 1965, 'Egy katona humoros kalandjai a második világháború alatt.', 4, 3, 'Tizedes meg a többiek'),
(105, 1971, 'Egy szerelmi történet a politikai elnyomás árnyékában.', 5, 1, 'Szerelem'),
(106, 1986, 'Egy humoros animációs történet a macskák és egerek harcáról.', 6, 4, 'Macskafogó'),
(107, 1981, 'Egy zenekar tagjainak küzdelmei a rendszer ellen.', 7, 5, 'Kopaszkutya'),
(108, 1968, 'A magyar történelem egyik legnagyobb csatája.', 8, 6, 'Egri csillagok'),
(109, 1976, 'Egy filozófiai dráma az emberi erkölcsről.', 9, 1, 'Az ötödik pecsét'),
(110, 1967, 'Egy csoport katona története az orosz forradalom idején.', 10, 6, 'Csillagosok, katonák'),
(111, 1979, 'Egy humoros történet egy falusi bakter életéről.', 11, 3, 'Indul a bakterház'),
(112, 1931, 'Egy újgazdag család és az elegáns lakáj története.', 12, 3, 'Hyppolit, a lakáj'),
(113, 1989, 'Két nővérek élete a 20. század elején.', 13, 1, 'Az én XX. századom'),
(114, 2015, 'Egy fiatal nő szerelmi története, némi misztikummal.', 14, 4, 'Liza, a rókatündér'),
(115, 2003, 'Egy metróellenőr mindennapjai és kihívásai.', 15, 1, 'Kontroll'),
(116, 2006, 'Egy szürreális családi történet több generáción keresztül.', 16, 1, 'Taxidermia'),
(117, 2002, 'Széchenyi István életének drámai ábrázolása.', 17, 6, 'Hídember'),
(118, 2002, 'Egy falusi közösség csendes története a mindennapokról.', 18, 1, 'Hukkle'),
(119, 1966, 'Egy fiú és az apja kapcsolata a múlt árnyékában.', 19, 1, 'Apa'),
(120, 1969, 'Egy fiúcsapat barátságának története.', 20, 7, 'Pál utcai fiúk'),
(121, 2001, 'A rendszerváltás idején játszódó fiatalos történet.', 21, 1, 'Moszkva tér'),
(122, 2018, 'Egy nő túlélési története egy szovjet munkatáborban.', 22, 6, 'Örök tél'),
(123, 2006, 'Az 1956-os forradalom története és annak hatásai.', 23, 6, 'Szabadság, szerelem'),
(124, 1991, 'Egy humoros történet a rendszerváltás idejéről.', 24, 3, 'Csapd le csacsi!'),
(125, 1984, 'Egy szürreális szerelmi történet.', 25, 1, 'Eszkimó asszony fázik'),
(126, 2004, 'Egy humoros történet a magyar történelem jelentős eseményeiről.', 26, 3, 'Magyar vándor'),
(127, 2017, 'Két lélek különös kapcsolata egy vágóhídon.', 27, 1, 'Testről és lélekről'),
(128, 2014, 'Egy kóbor kutya története a modern társadalomban.', 28, 1, 'Fehér isten'),
(129, 1918, 'Egy klasszikus Jókai Mór regény adaptációja.', 29, 1, 'Aranyember'),
(130, 1993, 'Egy humoros és nosztalgikus történet az életről.', 30, 1, 'Sose halunk meg');
